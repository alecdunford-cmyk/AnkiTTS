...
class AppSettings:
    ...
    def validate(self) -> None:
        ...
        if not isinstance(self.front_language, str) or not self.front_language:
            raise ValueError("front_language must be a non-empty string.")
        ...
        for attribute_name in ("rate", "volume", "pitch"):
    ...
    def from_dict(cls, data: dict[str, Any]) -> AppSettings:
        ...
        if isinstance(data.get("front_language"), str):
            settings.front_language = data["front_language"]

        if isinstance(data.get("voices"), dict):
            settings.voices.update(
                {
                    str(language): str(voice)
                    for language, voice in data["voices"].items()
                }
            )

        if isinstance(data.get("rate"), str):
            settings.rate = data["rate"]

        if isinstance(data.get("volume"), str):
            settings.volume = data["volume"]

        if isinstance(data.get("pitch"), str):
...
class SettingsManager:
    ...
    def load(self) -> AppSettings:
        ...
        try:
            ...
            print(
                f"Could not load settings from {self.settings_path}: {error}"
            )
